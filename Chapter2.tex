...
The serverless messaging layer offers a single operation, \textbf{post}(AR message), where the AR message contains the profile that is used to route the message. To route a message using the post abstraction the following steps are performed:

\begin{itemize}
    \item Step 1: The location specified in the AR message is used to the location aware layer which is responsible for performing a location query in the quadtree. The result of the query is the P2P network that will recive the message.
...
\begin{enumerate}
  \item \textbf{AND} concatenates two or more conditions.
  \item \textbf{OR}  
  \item \textbf{NOT} 
  \item \textbf{$>$,$<$,$>=$,$<=$} evaluates if the object is bigger, smaller, bigger or equal, smaller or equal than the value.
  \item \textbf{==} evaluates if the object is the same as the value.
  \item \textbf{AVG}
  \item \textbf{MIN}
  \item \textbf{MAX}
